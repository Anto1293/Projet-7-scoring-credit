...
# Fonction pour la jauge de score
...
if selected_menu == "Vue client":
    ...
    st.plotly_chart(plot_risk_gauge(prediction['proba']), use_container_width=True)

    with st.expander("Variables catégorielles OHE"):
        row = pd.Series(client_data)
...
# ----- Comparaison -----
    ...
    client_ids = get_all_client_ids()
    selected_id = st.selectbox("Sélection ID client pour comparaison", client_ids)
    client_data = get_client_data(selected_id)
    original_data = pd.DataFrame([client_data])
    client_df = pd.DataFrame([client_data])
    client_age = client_data['AGE']
    client_income = client_data['AMT_INCOME_TOTAL']
    feature_x = st.selectbox("Variable en X", ["AGE", "AMT_INCOME_TOTAL", "EXT_SOURCE_1"])
    feature_y = st.selectbox("Variable en Y", ["EXT_SOURCE_2", "EXT_SOURCE_3", "AMT_CREDIT"])

    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=df_all_clients[feature_x],
        y=df_all_clients[feature_y],
        mode="markers",
        name="Population",
        marker=dict(color="lightgrey", size=5), opacity=0.4))
    fig.add_trace(go.Scatter(
        x=[client_df[feature_x].values[0] if feature_x in client_df else client_age],
        y=[client_df[feature_y].values[0] if feature_y in client_df else client_income],
        mode="markers+text",
        name="Client",
        text=["Client"],
    ...
    fig.update_layout(title=f"Comparaison bivariée : {feature_x} vs {feature_y}",
